...
import { CheckCircle, AlertCircle, Info, Bot } from "lucide-react"
...
export function ActionResultDisplay({ result, isVisible, className }: ActionResultProps) {
    ...
    >
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 space-y-2">
          ...
          {result.action && <p className="text-xs text-muted-foreground">Action: {result.action.replace("_", " ")}</p>}
        </div>
      </div>
    </Card>
  )
